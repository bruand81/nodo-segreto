...
**Implemented.** Flutter/Dart app, scaffolded for Android, iOS, macOS, Windows and Linux. All three required ciphers (Morse, Cesare, sostituzione numerica) plus the optional Pigpen cipher are implemented and tested. Storage, sharing/QR, Scout theme, branding and the About page are done too. Windows/Linux builds have not been exercised (developed on macOS); macOS, iOS (simulator) and Android (emulator) have all been built and run successfully — verified end-to-end on 2026-07-26 while producing App Store/Play Store screenshots (workflow and gotchas logged in the local, untracked `istruzioni_store.md`). iOS and macOS were migrated from CocoaPods to Swift Package Manager on 2026-08-15 (see the "Comandi"/"Architettura" notes below); the app was also built, installed and run on a real iPhone (physical hardware, iOS 27.0) for the first time that day, confirming the SPM-based build works outside the simulator. The app was also built, installed and run on a real Samsung SM G965F (Android 10, API 29) for the first time that same day, confirming the app works on physical Android hardware — this required fixing a JDK/Kotlin build incompatibility on this machine first (see "Comandi" note on `openjdk@21`). La scansione QR dal vivo (fotocamera) è stata verificata funzionante su un iPhone fisico reale ("iAntaresPhone17", iOS 27.0) e su un Samsung SM G965F fisico (Android 10, API 29) il 2026-08-15.
...
- **Sharing**: messages can be shared via file export, native share sheet, or QR code (generate + import from image, or live camera scan on iOS/Android).
- **Input methods**: manual text entry, import from a text file, import from a QR code image, or live QR camera scan (iOS/Android only).
...
- `flutter run -d <device-id>` su un iPhone fisico collegato via **Wi-Fi (wireless debugging)** può bloccarsi indefinitamente su "The Dart VM Service was not discovered..." e l'app resta ferma sullo splash screen: il debug wireless su iOS 27 è lento/inaffidabile. Fix: collegare il dispositivo via cavo USB (`flutter devices` lo mostra senza l'etichetta "(wireless)") e rilanciare — il primo avvio via USB può comunque impiegare fino a ~80s prima che il Dart VM Service si connetta, è normale.
- Se la build Android fallisce con `FAILURE: Build failed with an exception` / `* What went wrong: 25.0.2` (o `26.0.2`) e uno stacktrace `java.lang.IllegalArgumentException` in `org.jetbrains.kotlin...JavaVersion.parse`: il parser di versione Java incluso in Kotlin 2.2.20 non riconosce gli JDK 25/26 (gli unici disponibili su questa macchina, sia quello di sistema sia quello incluso in Android Studio). Fix: `brew install openjdk@21`, poi `org.gradle.java.home=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home` in `android/gradle.properties` (già impostato — se si riclona/reinstalla su un'altra macchina, adattare il path o rimuovere la riga se lì è già presente un JDK ≤21).
...
- **Sharing**: export file, condivisione (share sheet) e QR condividono sempre il **testo cifrato così com'è** (l'output mostrato in UI), non un envelope — chi riceve legge/decodifica scegliendo lui stesso cifrario e configurazione nell'app (coerente con l'uso da gioco scout). `share_service.dart` (export/import file + share sheet nativo, mimetype `text/plain`), `qr_decode_service.dart` (decodifica QR da immagine via `mobile_scanner.analyzeImage`), `qr_display_dialog.dart` (generazione QR via `pretty_qr_code`), `qr_scan_screen.dart` (scansione QR dal vivo via il widget `MobileScanner`, solo iOS/Android — gated da `_supportsLiveQrScan` in `home_page.dart`; su desktop resta solo l'import da immagine). `lib/core/sharing/share_envelope.dart` (JSON: `{app, v, cipherId, direction, config, text}`) resta solo come formato d'**importazione opzionale**: se un file/QR importato (da immagine o da scansione live) è un envelope valido, `HomePage._applyImportedContent` ripristina anche cifrario e config; altrimenti il contenuto viene trattato come testo semplice.
...
- La scansione QR dal vivo (fotocamera, `qr_scan_screen.dart`) è cablata solo su iOS/Android (`_supportsLiveQrScan` in `home_page.dart`); su macOS/Windows/Linux resta solo l'import da immagine già scelta (`mobile_scanner.analyzeImage`). Su Windows/Linux `mobile_scanner` non è supportato nemmeno per quel percorso: andrebbe aggiunto un fallback (es. `zxing2` + `image`, pure Dart) se servirà importare QR su quelle piattaforme. La scansione live non è testata da `flutter test` (nessun canale di piattaforma camera disponibile in test) — verificata solo manualmente su dispositivo/simulatore.
